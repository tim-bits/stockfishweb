...
public class Query {
    ...
    private static final int MAX_TIME = 1000; //4 sec
    ...
    public Query(QueryType type, String fen) {
        this.type = type;
        this.fen = fen;
    }

    public Query(QueryType type, String fen, int depth) {
        this.type = type;
        this.fen = fen;
        this.depth = depth;
    }
    ...
    public static class Builder {
        ...
        public static final String FEN_REGEX = "(([rnbqkp1-8PRNBQK]{1,8}/){7}[rnbqkp1-8PRNBQK]{1,8})" +
                "(\\s)([wb])(\\s[-kqKQ]{1,4}\\s)((-)|[a-h][1-8])(\\s)([0-9]+)(\\s)([0-9]+)";
        public static final String MOVE_REGEX = "([a-h][1-8]){2}";

        public static final Pattern MOVE_REGEX_PATTERN = Pattern.compile(MOVE_REGEX);

        private static final Pattern fenPattern = Pattern.compile(START_REGEX + FEN_REGEX + END_REGEX);
        public static final Pattern MOVE_PATTERN = Pattern.compile(START_REGEX + MOVE_REGEX + END_REGEX);
        private QueryType type;
        private String fen, move;
        ...
        public Builder setMove(String move) throws IllegalArgumentException {
            if (move == null || !MOVE_PATTERN.matcher(move).matches()) {
